...
void CCamera::MoveCamWithMouse( vec3f playerPos, HWND wnd)
{
	...
	tempos = CGame::GetInstance()->GetScreenHeight() - tempos;
...
// Mod. Date: 7/21/12
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
void CCamera::SetInterpolateFuturePosition( vec3f PosTo )
{
	// Grab the Cursors Position as a vec3f.
	vec3f tCursorPos = *((vec3f*)(&m_pInput->GetCursorPosition()));
	...
	//Sets the players pos var
	vec3f xPlayerPos = PosTo;

	//Calculate the Cameras Position. Camera is moved by mouse cuz of this equation
	FutureCameraPositionForInterpolation = ( ( xPlayerPos + ( vec3f( tCursorPos.x, tempos, 0.0f ) / 2.0f )) );

	//Cursor in world space calculations 
	m_Cursor = vec3f( (float)(tCursorPos.x), tempos, 0.0f ) * 2.0f + vec3f( xPlayerPos.x, xPlayerPos.y, xPlayerPos.z );

	//Camera offset. Aka zoom in or out
	FutureCameraPositionForInterpolation += vec3f( 0.0f, 0.0f, -1000.0f );
...
// Mod. Date: 7/21/12
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
bool CCamera::InterpolateCameraOverTime( float _fDT, vec3f PosTo )
{
	// Increment the Timer.
	InterpolateLambda += _fDT;
	if(InterpolateLambda > 1.0f )
		InterpolateLambda = 1.0f;

	// Interpolate from the Current Camera Position to the Future Camera Position.
	vec3f NewPosition = interpolate(CurrentCameraPositionForInterpolation, FutureCameraPositionForInterpolation, InterpolateLambda );

	SetViewPosition( (*(D3DXVECTOR3*)&NewPosition) );

	// If Lambda is 1 then we have finished Interpolating.
	if( InterpolateLambda == 1.0f )
		return true;
