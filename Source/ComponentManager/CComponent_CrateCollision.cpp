...
void CComponent_CrateCollision::Update( float fDT )
{
	...
	int ReturnParams = 0;
	int ReturnBody = 0;
	int ReturnObjects =  (1<<OBJ_CRATE) | (1<<OBJ_CORK) | (1<<OBJ_PLUG) | (1<<OBJ_WORLD_COLLISION) | (1<<OBJ_WALLCHUNK);
	...
	//check to see if we were told that we are colliding any other way this frame
	if(!m_bCol)
	{
		m_bCol = m_bCollidingGround;
	}

	if(!m_bCol)
	{
		m_bCol = m_bCollidingWall;
	}
	if(m_pSoundComponent && (!m_bPrevCol && m_bCol) && m_fPlayTimer > 0.1f)
	{
		float amount_moved = abs(dot_product(m_tPrevPos - m_pPhysicsBox->GetCentroid(), m_tPrevPos - m_pPhysicsBox->GetCentroid()));
		if(amount_moved > 1.0f)
		{
			m_pSoundComponent->PlaySound(AK::EVENTS::PLAY_SFX_CRATE_COLLIDE);
			m_fPlayTimer = 0.0f;
		}

		m_tPrevPos = m_pPhysicsBox->GetCentroid();
	}
