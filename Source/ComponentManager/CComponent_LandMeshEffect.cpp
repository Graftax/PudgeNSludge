...
CComponent_LandMeshEffect::CComponent_LandMeshEffect(void) : IComponent(ECOMP_MESHEFFECT)
{
	m_pGame = nullptr;
...
bool CComponent_LandMeshEffect::Initialize()
{
	...
	m_pCamera = m_pGame->GetCamera();
		...
		if( m_pPlayer )
		{
			m_pPlayerCol = (CComponent_PlayerCollision*)m_pPlayer->GetComponent(ECOMP_COLLISION);
			m_pPlayerHealth = (CComponent_Health*)m_pPlayer->GetComponent(ECOMP_HEALTH);
		}
	return true;
}
...
{	
	...
	if( m_pPlayerCol )
	{
		m_bCollidingGround = m_pPlayerCol->GetCollidingGroundAlone();
		isCollidingLast = m_pPlayerCol->GetCollidingGroundLast();
	}

	//If player is not dead. Allow player to Land 
	if( m_pPlayerHealth->GetHealth() >= 0.1f)
	{
