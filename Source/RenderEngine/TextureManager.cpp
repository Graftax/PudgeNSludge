...
int TextureManager::LoadTexture( const char* _szFileName )
{
	...
	result = D3DXCreateTextureFromFileExA( 
		RenderEngine::m_pD3Dev, // The Device Associated with this Texture
		(LPCSTR)(_szFileName),	// The file name of the texture.
		D3DX_DEFAULT,			// Width in pixels. If this value is zero or D3DX_DEFAULT, the dimensions are taken from the file and rounded up to a power of two. 
								// If the device supports non-power of 2 textures and D3DX_DEFAULT_NONPOW2 is specified, the size will not be rounded.
		D3DX_DEFAULT,			// Same as above.
		D3DX_FROM_FILE,			// If this value is zero or D3DX_DEFAULT, a complete mipmap chain is created. If D3DX_FROM_FILE, the size will be taken exactly as it is in the file
		D3DUSAGE_RENDERTARGET,
		D3DFMT_UNKNOWN,
		D3DPOOL_DEFAULT,
		D3DX_DEFAULT,
		D3DX_DEFAULT,
		D3DCOLOR_ARGB(0,0,0,0),				// Set Black as the transparent color
		NULL,
		NULL,
		&m_umTextureMap[nId].m_lpTexture);


	if( FAILED( result/*result = D3DXCreateTextureFromFileA( RenderEngine::m_pD3Dev, (LPCSTR)(_szFileName), &m_umTextureMap[nId].m_lpTexture )*/))
	{
...
void TextureManager::UnloadTexture( unsigned int _uiIndex )
{
	...
	if( m_umTextureMap[_uiIndex].m_lpTexture )
	{
		unordered_map< unsigned int, textureData >::iterator iter( m_umTextureMap.begin() );
...
void TextureManager::ReloadAllTextures(void)
{
	for( unsigned int i = 0u; i < m_umTextureMap.size(); ++i )
	{
		...
		RenderEngine::m_pD3Dev, // The Device Associated with this Texture
		(LPCSTR)(m_umTextureMap[i].m_filePath),	// The file name of the texture.
		D3DX_DEFAULT,			// Width in pixels. If this value is zero or D3DX_DEFAULT, the dimensions are taken from the file and rounded up to a power of two. 
								// If the device supports non-power of 2 textures and D3DX_DEFAULT_NONPOW2 is specified, the size will not be rounded.
		D3DX_DEFAULT,			// Same as above.
		D3DX_FROM_FILE,			// If this value is zero or D3DX_DEFAULT, a complete mipmap chain is created. If D3DX_FROM_FILE, the size will be taken exactly as it is in the file
