...
void PostProcess::Process( RenderTarget &_destination, LPDIRECT3DBASETEXTURE9 _sourceTex )
{
	...
	passHandle = m_pEffect->GetPass("PostProcess", 0 );
	scaleHandle = m_pEffect->GetAnnotationByName(passHandle, "fScaleX" );
	if( scaleHandle )
		m_pEffect->GetFloat(scaleHandle, &fScaleX );
	scaleHandle = m_pEffect->GetAnnotationByName(passHandle, "fScaleY" );
	if( scaleHandle )
		m_pEffect->GetFloat(scaleHandle, &fScaleY );

	D3DXMatrixScaling( &_matProj, fScaleX, fScaleY, 1.0f );
	...
	HRESULT hr = m_pEffect->SetTexture( m_hTexture, _sourceTex );
