...
void RenderEngine::DrawGBuffers( RenderNode& pNode )
{
	...
	if(GBufferEffect)
	{
		...
		{
			float fScrollModifier = 1.0f;
			if( pNode.m_pOwner->TestFlag( RE_BURNTEX ) )
			{
				GBufferEffect->SetTechnique( "GBuffersDissolve" );
				GBufferEffect->SetTexture( "dissolveMap", m_pTextureMan->m_umTextureMap[m_pDissolveTexture].m_lpTexture );
				GBufferEffect->SetTexture( "dissolveToMap", m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[BURNDOWN_TEX]].m_lpTexture );
				if( pNode.m_pOwner->TestFlag( RE_BURNOWNER ) )
				{
					GBufferEffect->SetFloat( "dissolveFactor", pNode.m_pOwner->m_fExtraData);
				}
				else
				{
					pNode.m_pOwner->m_fExtraData -= BURN_RATE;
					GBufferEffect->SetFloat( "dissolveFactor", pNode.m_pOwner->m_fExtraData);
				}
			}
			else
			{
				...
				fScrollModifier = pNode.m_pOwner->GetExtraData();
			}
			...
			if( pNode.m_pOwner->TestFlag( RE_UVS_NONE ) )
				...
				GBufferEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( -(m_v2fScrollOffset.x * fScrollModifier), -(m_v2fScrollOffset.y * fScrollModifier), 0.0f, 0.0f ) );
			else if( pNode.m_pOwner->TestFlag( RE_UVS_NX ) )
				GBufferEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( -(m_v2fScrollOffset.x * fScrollModifier), 0.0f, 0.0f, 0.0f ) );
			else if( pNode.m_pOwner->TestFlag( RE_UVS_NY ) )
				GBufferEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( 0.0f, -(m_v2fScrollOffset.y * fScrollModifier), 0.0f, 0.0f ) );
			...
			GBufferEffect->SetTexture( "diffuseMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[0]].m_lpTexture));
			GBufferEffect->SetTexture( "normalMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[1]].m_lpTexture));
			GBufferEffect->SetTexture( "specularMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[2]].m_lpTexture));
			GBufferEffect->SetTexture( "emissiveMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[0]].m_lpTexture));
		...
		else
		{
			...
			{
				CCollisionAABB aabb = pNode.GetRenderData().m_vCollisions[0];
				D3DXMATRIX pos = pNode.renderData.m_vTransform[pNode.renderData.m_vTransform.size()-1];
				aabb.SetWorldPoint( vec3f(pos._41, pos._42, pos._43));
				if( !ViewFrustum.infrustum(  aabb ))
				{
					return;
				}

				if( pNode.m_pOwner->TestFlag( RE_PLAYER ) )
				{
					AnimatedMeshEffect->SetTechnique("SkinningGBuffersFade");
					AnimatedMeshEffect->SetFloat( "fadeFactor", (m_fPlayerHealth / 100.0f) );
					AnimatedMeshEffect->SetTexture( "dissolveMap", TextureManager::GetInstance()->m_umTextureMap[m_pDissolveTexture].m_lpTexture );
					AnimatedMeshEffect->SetTexture( "fadeDiffuseMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[5]].m_lpTexture));
					AnimatedMeshEffect->SetTexture( "fadeNormalMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[6]].m_lpTexture));
					AnimatedMeshEffect->SetTexture( "fadeSpecularMap", (m_pTextureMan->m_umTextureMap[pNode.m_pOwner->renderData.m_uiTextureHandles[7]].m_lpTexture));
				}
				else
				...
				for( UINT i = 0; i < passes; i++)
				{
					AnimatedMeshEffect->BeginPass(i);
					{
						...
						GBufferEffect->SetTexture( "emissiveMap", (m_pTextureMan->m_umTextureMap[pNode.renderData.m_uiTextureHandles[0]].m_lpTexture));
...
void RenderEngine::DrawTexturedEnvironmentMesh( RenderNode& pNode )
{
	...
	if(TextureMeshEffect)
	{
		...
		if( ViewFrustum.infrustum(  aabb ))
		{
			...
			{
				pNode.m_pOwner->m_fExtraData -= BURN_RATE;
				if( pNode.m_pOwner->m_fExtraData <= -0.02f )
				{
				...
				if(TextureMeshEffect)
				{
					...
					if( pNode.m_pOwner->TestFlag( RE_UVS_NONE ) )
					{
						...
						TextureMeshEffect->SetFloat( "dissolveFactor", pNode.m_pOwner->m_fExtraData );
			...
			{
				float fScrollModifier = pNode.GetExtraData();
				...
				if( pNode.m_pOwner->TestFlag( RE_UVS_NONE ) )
					...
					TextureMeshEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( -(m_v2fScrollOffset.x * fScrollModifier), -(m_v2fScrollOffset.y * fScrollModifier), 0.0f, 0.0f ) );
				else if( pNode.m_pOwner->TestFlag( RE_UVS_NX ) )
					TextureMeshEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( -(m_v2fScrollOffset.x * fScrollModifier), 0.0f, 0.0f, 0.0f ) );
				else if( pNode.m_pOwner->TestFlag( RE_UVS_NY ) )
					TextureMeshEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( 0.0f, -(m_v2fScrollOffset.y * fScrollModifier), 0.0f, 0.0f ) );
				else if( pNode.m_pOwner->TestFlag( RE_UVS_PB ) )
					TextureMeshEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( (m_v2fScrollOffset.x * fScrollModifier), (m_v2fScrollOffset.y * fScrollModifier), 0.0f, 0.0f ) );
				else if( pNode.m_pOwner->TestFlag( RE_UVS_PX ) )
					TextureMeshEffect->SetVector( "gTextureOffset", &D3DXVECTOR4( (m_v2fScrollOffset.x * fScrollModifier), 0.0f, 0.0f, 0.0f ) );
...
{
	pNode.m_pOwner->m_fExtraData -= BURN_RATE;
	if( pNode.m_pOwner->m_fExtraData <= -0.02f && !pNode.m_pOwner->TestFlag( RE_BURNTEX ) )
	{
		pNode.m_pOwner->AddFlags( RE_UVS_NONE | RE_BURNTEX );
	}
	//Drawing call used to draw a flat shaded textured mesh
	if(TextureMeshEffect)
	{
