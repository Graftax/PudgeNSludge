...
void LightManager::InitLightManager(void)
{
	...
	m_fBrightness = 0.0f;
	m_fTimeToRenderLights = 0.0f;
	m_bRaiseBrightness = true;
}
...
{
	RenderEngine::DirectionalLightEffect->SetFloat( "gBrightness", m_fBrightness );
			...
			case LIGHT_POINT:
				{
					...
					_ToAddPL->GetCollisionSphere().SetCenter( vec3f( position[0], position[1], position[2] ) );
					_ToAddPL->GetCollisionSphere().SetRadius( radius );
			...
			case LIGHT_SPOT:
				{
					...
					_ToAddSL->GetCollisionSphere().SetCenter( vec3f( position[0], position[1], position[2] ) );
					_ToAddSL->GetCollisionSphere().SetRadius( radius * 3.0f );
...
void LightManager::Update(float _fDT )
{
	if( m_bRaiseBrightness )
	{
		m_fTimeToRenderLights += _fDT * 0.7f;
		if( m_fTimeToRenderLights > 1.0f )
			m_fTimeToRenderLights = 1.0f;
	}
	else
	{
		m_fTimeToRenderLights -= _fDT * 0.7f;
		if( m_fTimeToRenderLights < 0.0f )
			m_fTimeToRenderLights = 0.0f;
	}

	m_fBrightness = (m_fTimeToRenderLights / 1.0f);
}
