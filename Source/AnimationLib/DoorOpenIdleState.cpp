...
DoorOpenIdleState::~DoorOpenIdleState(void)
{
}

// Access the state to set it in the state machine for each component.
DoorOpenIdleState* DoorOpenIdleState::GetState(void)
{
	static DoorOpenIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void DoorOpenIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Door_OpenIdle" ) );
...
void DoorOpenIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void DoorOpenIdleState::Exit( CComponent_Animation* pComponent )
{
