...
BridgeHalfRetractState::~BridgeHalfRetractState(void)
{
}

// Access the state to set it in the state machine for each component.
BridgeHalfRetractState* BridgeHalfRetractState::GetState(void)
{
	static BridgeHalfRetractState state;
	return &state;
}

// This will set all variables needed for this animation.
void BridgeHalfRetractState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Bridge_HalfRetract" ) );
...
void BridgeHalfRetractState::Execute( CComponent_Animation* pComponent, float fTime )
{
	// Switch to idle if this animation is done
	if( pComponent->GetInterpolator().IsAnimationDone() )
	{
		pComponent->GetAnimStateMachine()->ChangeState( BridgeHalfRetractIdleState::GetState() );
	}
...
void BridgeHalfRetractState::Exit( CComponent_Animation* pComponent )
{
