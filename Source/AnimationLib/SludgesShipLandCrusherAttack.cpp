...
SludgesShipLandCrusherAttackState::~SludgesShipLandCrusherAttackState(void)
{
}

// Access the state to set it in the state machine for each component.
SludgesShipLandCrusherAttackState* SludgesShipLandCrusherAttackState::GetState(void)
{
	static SludgesShipLandCrusherAttackState state;
	return &state;
}

// This will set all variables needed for this animation.
void SludgesShipLandCrusherAttackState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("SludgesShip_LandCrusherAttack") );
...
void SludgesShipLandCrusherAttackState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void SludgesShipLandCrusherAttackState::Exit( CComponent_Animation* pComponent )
{
