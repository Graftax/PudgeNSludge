...
PudgeRunState::~PudgeRunState(void)
{
}

// Access the state to set it in the state machine for each component.
PudgeRunState* PudgeRunState::GetState()
{
	static PudgeRunState state;
	return &state;
}

// This will set all variables needed for this animation.
void PudgeRunState::Enter( CComponent_Animation* pComponent )
{
...
void PudgeRunState::Execute( CComponent_Animation* pComponent, float fTime )
{
	...
	for( unsigned int i = 0; i < pComponent->GetAnimationFrames().size(); ++i)
		pComponent->GetAnimationFrames()[i].GetWorldMat() = (*(matrix4f*)&pComponent->GetBindPose()->GetHierarchyList()[i]->m_InverseLocalMatrix) * pComponent->GetInterpolator().GetCurrentBoneFrames()[i].GetWorldMat();
}

// Clean up of the Animation.
void PudgeRunState::Exit( CComponent_Animation* pComponent )
{
