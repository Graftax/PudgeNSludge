...
PullEffectPullForeverState::~PullEffectPullForeverState(void)
{
}

// Access the state to set it in the state machine for each component.
PullEffectPullForeverState* PullEffectPullForeverState::GetState(void)
{
	static PullEffectPullForeverState state;
	return &state;
}

// This will set all variables needed for this animation.
void PullEffectPullForeverState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("PullMesh_PullForever") );
...
void PullEffectPullForeverState::Execute( CComponent_Animation* pComponent, float fTime )
{
	...
	//	pComponent->GetAnimStateMachine()->ChangeState( PullEffectIdleState::GetState() );
}

// Clean up of the Animation.
void PullEffectPullForeverState::Exit( CComponent_Animation* pComponent )
{
