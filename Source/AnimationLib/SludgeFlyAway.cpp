...
SludgeFlyAway::~SludgeFlyAway(void)
{
}

// Access the state to set it in the state machine for each component.
SludgeFlyAway* SludgeFlyAway::GetState(void)
{
	static SludgeFlyAway state;
	return &state;
}

// This will set all variables needed for this animation.
void SludgeFlyAway::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Sludge_FlyAway") );
...
void SludgeFlyAway::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void SludgeFlyAway::Exit( CComponent_Animation* pComponent )
{
