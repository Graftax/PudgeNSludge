...
BridgeIdleState::~BridgeIdleState(void)
{
}

// Access the state to set it in the state machine for each component.
BridgeIdleState* BridgeIdleState::GetState(void)
{
	static BridgeIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void BridgeIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Bridge_Idle" ) );
...
void BridgeIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void BridgeIdleState::Exit( CComponent_Animation* pComponent )
{
