...
PudgeIdleState::~PudgeIdleState(void)
{
}

// Access the state to set it in the state machine for each component.
PudgeIdleState* PudgeIdleState::GetState(void)
{
	static PudgeIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void PudgeIdleState::Enter( CComponent_Animation* pComponent )
{
...
void PudgeIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void PudgeIdleState::Exit( CComponent_Animation* pComponent )
{
