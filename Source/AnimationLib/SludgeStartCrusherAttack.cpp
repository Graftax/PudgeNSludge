...
SludgeStartCrusherAttackState::~SludgeStartCrusherAttackState(void)
{
}

// Access the state to set it in the state machine for each component.
SludgeStartCrusherAttackState* SludgeStartCrusherAttackState::GetState(void)
{
	static SludgeStartCrusherAttackState state;
	return &state;
}

// This will set all variables needed for this animation.
void SludgeStartCrusherAttackState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Sludge_StartCrusherAttack") );
...
void SludgeStartCrusherAttackState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void SludgeStartCrusherAttackState::Exit( CComponent_Animation* pComponent )
{
