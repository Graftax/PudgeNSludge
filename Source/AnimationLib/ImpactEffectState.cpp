...
ImpactEffectState::~ImpactEffectState(void)
{
}

// Access the state to set it in the state machine for each component.
ImpactEffectState* ImpactEffectState::GetState(void)
{
	static ImpactEffectState state;
	return &state;
}

// This will set all variables needed for this animation.
void ImpactEffectState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("ImpactMesh_Impact") );
...
void ImpactEffectState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void ImpactEffectState::Exit( CComponent_Animation* pComponent )
{
