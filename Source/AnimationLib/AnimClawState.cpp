...
AnimClawState::~AnimClawState(void)
{
}

// Access the state to set it in the state machine for each component.
AnimClawState* AnimClawState::GetState(void)
{
	static AnimClawState state;
	return &state;
}

// This will set all variables needed for this animation.
void AnimClawState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Claw_Run" ) );
...
void AnimClawState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void AnimClawState::Exit( CComponent_Animation* pComponent )
{
