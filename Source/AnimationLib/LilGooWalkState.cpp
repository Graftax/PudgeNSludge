...
LilGooWalkState::~LilGooWalkState(void)
{
}

// Access the state to set it in the state machine for each component.
LilGooWalkState* LilGooWalkState::GetState(void)
{
	static LilGooWalkState state;
	return &state;
}

// This will set all variables needed for this animation.
void LilGooWalkState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("LilGoo_Walk") );
...
void LilGooWalkState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void LilGooWalkState::Exit( CComponent_Animation* pComponent )
{
