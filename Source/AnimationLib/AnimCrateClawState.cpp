...
AnimCrateClawState::~AnimCrateClawState(void)
{
}

// Access the state to set it in the state machine for each component.
AnimCrateClawState* AnimCrateClawState::GetState(void)
{
	static AnimCrateClawState state;
	return &state;
}

// This will set all variables needed for this animation.
void AnimCrateClawState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "CrateClaw_Movement" ) );
...
void AnimCrateClawState::Execute( CComponent_Animation* pComponent, float fTime )
{
	if( pComponent->GetInterpolator().IsAnimationDone() )
	{
		if( pComponent->GetInterpolator().GetTime() >= const_cast<Animation*>(pComponent->GetInterpolator().GetAnimation())->GetDuration() )
		{
			pComponent->GetInterpolator().SetTime( 0.0f );
...
void AnimCrateClawState::Exit( CComponent_Animation* pComponent )
{
