...
JumpEffectState::~JumpEffectState(void)
{
}

// Access the state to set it in the state machine for each component.
JumpEffectState* JumpEffectState::GetState(void)
{
	static JumpEffectState state;
	return &state;
}

// This will set all variables needed for this animation.
void JumpEffectState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("JumpSteamStartMesh_JumpStart") );
...
void JumpEffectState::Execute( CComponent_Animation* pComponent, float fTime )
{
	...
	for( unsigned int i = 0; i < pComponent->GetAnimationFrames().size(); ++i)
 	...
 	{
 		pComponent->GetAnimStateMachine()->ChangeState( JumpIdleEffectState::GetState() );
 		//	LOG(LOG_INFO, "PushEffectState::Execute()", "Push changed to idle");
 	}
}

// Clean up of the Animation.
void JumpEffectState::Exit( CComponent_Animation* pComponent )
{
