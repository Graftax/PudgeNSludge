...
LilGooFlyingState::~LilGooFlyingState(void)
{
}

// Access the state to set it in the state machine for each component.
LilGooFlyingState* LilGooFlyingState::GetState(void)
{
	static LilGooFlyingState state;
	return &state;
}

// This will set all variables needed for this animation.
void LilGooFlyingState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("LilGoo_Flying") );
...
void LilGooFlyingState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void LilGooFlyingState::Exit( CComponent_Animation* pComponent )
{
