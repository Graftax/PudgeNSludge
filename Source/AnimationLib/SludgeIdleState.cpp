...
SludgeIdleState::~SludgeIdleState(void)
{
}

// Access the state to set it in the state machine for each component.
SludgeIdleState* SludgeIdleState::GetState(void)
{
	static SludgeIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void SludgeIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Sludge_Idle") );
...
void SludgeIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void SludgeIdleState::Exit( CComponent_Animation* pComponent )
{
