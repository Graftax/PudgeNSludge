...
BigHurtEffectState::~BigHurtEffectState(void)
{
}

// Access the state to set it in the state machine for each component.
BigHurtEffectState* BigHurtEffectState::GetState(void)
{
	static BigHurtEffectState state;
	return &state;
}

// This will set all variables needed for this animation.
void BigHurtEffectState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("BigHurtMesh_Hurt") );
...
void BigHurtEffectState::Execute( CComponent_Animation* pComponent, float fTime )
{
	...
	{
		pComponent->GetAnimStateMachine()->ChangeState( BigHurtIdleEffectState::GetState() );
		//	LOG(LOG_INFO, "PushEffectState::Execute()", "Push changed to idle");
	}
}

// Clean up of the Animation.
void BigHurtEffectState::Exit( CComponent_Animation* pComponent )
{
