...
PudgeIdleToJumpState::~PudgeIdleToJumpState(void)
{
}

// Access the state to set it in the state machine for each component.
PudgeIdleToJumpState* PudgeIdleToJumpState::GetState(void)
{
	static PudgeIdleToJumpState state;
	return &state;
}

// This will set all variables needed for this animation.
void PudgeIdleToJumpState::Enter( CComponent_Animation* pComponent )
{
	m_fTimeInState = 0.0f;
	
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_Idle") );
	pComponent->GetBlendInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_Jump") );
...
void PudgeIdleToJumpState::Execute( CComponent_Animation* pComponent, float fTime )
{
	...
	{
		pComponent->GetAnimStateMachine()->ChangeState( PudgeJumpState::GetState() );
	}
}

// Clean up of the Animation.
void PudgeIdleToJumpState::Exit( CComponent_Animation* pComponent )
{
