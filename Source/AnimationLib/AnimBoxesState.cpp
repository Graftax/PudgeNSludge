...
AnimBoxesState::~AnimBoxesState(void)
{
}

// Access the state to set it in the state machine for each component.
AnimBoxesState* AnimBoxesState::GetState(void)
{
	static AnimBoxesState state;
	return &state;
}

// This will set all variables needed for this animation.
void AnimBoxesState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Boxes_Run" ) );
...
void AnimBoxesState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void AnimBoxesState::Exit( CComponent_Animation* pComponent )
{
