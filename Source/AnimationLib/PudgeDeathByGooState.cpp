...
PudgeDeathByGooState::~PudgeDeathByGooState(void)
{
}

// Access the state to set it in the state machine for each component.
PudgeDeathByGooState* PudgeDeathByGooState::GetState(void)
{
	static PudgeDeathByGooState state;
	return &state;
}

// This will set all variables needed for this animation.
void PudgeDeathByGooState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetIsAnimationDone( false );

	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_DeathByGoo") );
...
void PudgeDeathByGooState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void PudgeDeathByGooState::Exit( CComponent_Animation* pComponent )
{
