...
DoorIdleState::~DoorIdleState(void)
{

}

// Access the state to set it in the state machine for each component.
DoorIdleState* DoorIdleState::GetState(void)
{
	static DoorIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void DoorIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Door_Idle" ) );
...
void DoorIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void DoorIdleState::Exit( CComponent_Animation* pComponent )
{
