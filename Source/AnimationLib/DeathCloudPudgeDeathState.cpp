...
DeathCloudPudgeDeathState::~DeathCloudPudgeDeathState(void)
{
}

// Access the state to set it in the state machine for each component.
DeathCloudPudgeDeathState* DeathCloudPudgeDeathState::GetState(void)
{
	static DeathCloudPudgeDeathState state;
	return &state;
}

// This will set all variables needed for this animation.
void DeathCloudPudgeDeathState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("DeathCloud_PudgeDead") );
...
void DeathCloudPudgeDeathState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void DeathCloudPudgeDeathState::Exit( CComponent_Animation* pComponent )
{
