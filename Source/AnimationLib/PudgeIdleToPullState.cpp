...
PudgeIdleToPullState::~PudgeIdleToPullState(void)
{
}

// Access the state to set it in the state machine for each component.
PudgeIdleToPullState* PudgeIdleToPullState::GetState(void)
{
	static PudgeIdleToPullState state;
	return &state;
}

// This will set all variables needed for this animation.
void PudgeIdleToPullState::Enter( CComponent_Animation* pComponent )
{
	m_fTimeInState = 0.0f;

	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_Idle") );
	pComponent->GetBlendInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_Pull") );
...
void PudgeIdleToPullState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void PudgeIdleToPullState::Exit( CComponent_Animation* pComponent )
{
