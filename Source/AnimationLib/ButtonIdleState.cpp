...
ButtonIdleState::~ButtonIdleState(void)
{

}

// Access the state to set it in the state machine for each component.
ButtonIdleState* ButtonIdleState::GetState(void)
{
	static ButtonIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void ButtonIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "Button_Idle" ) );
...
void ButtonIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void ButtonIdleState::Exit( CComponent_Animation* pComponent )
{
