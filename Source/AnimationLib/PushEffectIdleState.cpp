...
PushEffectIdleState::~PushEffectIdleState(void)
{
}

// Access the state to set it in the state machine for each component.
PushEffectIdleState* PushEffectIdleState::GetState(void)
{
	static PushEffectIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void PushEffectIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("PushMesh_Idle") );
...
void PushEffectIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void PushEffectIdleState::Exit( CComponent_Animation* pComponent )
{
