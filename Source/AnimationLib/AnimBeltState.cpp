...
AnimBeltState::~AnimBeltState(void)
{
}

// Access the state to set it in the state machine for each component.
AnimBeltState* AnimBeltState::GetState(void)
{
	static AnimBeltState state;
	return &state;
}

// This will set all variables needed for this animation.
void AnimBeltState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation( "ConveyorBelt_Run" ) );
...
void AnimBeltState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void AnimBeltState::Exit( CComponent_Animation* pComponent )
{
