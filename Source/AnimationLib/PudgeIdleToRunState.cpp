...
PudgeIdleToRunState::~PudgeIdleToRunState(void)
{
}

// Access the state to set it in the state machine for each component.
PudgeIdleToRunState* PudgeIdleToRunState::GetState(void)
{
	static PudgeIdleToRunState state;
	return &state;
}

// This will set all variables needed for this animation.
void PudgeIdleToRunState::Enter( CComponent_Animation* pComponent )
{
	m_fTimeInState = 0.0f;

	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_Idle") );
	pComponent->GetBlendInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("Pudge_Run") );
...
void PudgeIdleToRunState::Execute( CComponent_Animation* pComponent, float fTime )
{
	...
	{
		pComponent->GetAnimStateMachine()->ChangeState( PudgeRunState::GetState() );
	}
}

// Clean up of the Animation.
void PudgeIdleToRunState::Exit( CComponent_Animation* pComponent )
{
