...
PullEffectIdleState::~PullEffectIdleState(void)
{
}

// Access the state to set it in the state machine for each component.
PullEffectIdleState* PullEffectIdleState::GetState(void)
{
	static PullEffectIdleState state;
	return &state;
}

// This will set all variables needed for this animation.
void PullEffectIdleState::Enter( CComponent_Animation* pComponent )
{
	pComponent->GetInterpolator().SetAnimation( AnimationManager::GetInstance()->GetAnimation("PullMesh_Idle") );
...
void PullEffectIdleState::Execute( CComponent_Animation* pComponent, float fTime )
{
...
void PullEffectIdleState::Exit( CComponent_Animation* pComponent )
{
