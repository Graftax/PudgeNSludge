...
void CCreditsState::Init(void)
{
	...
	if(m_pAudio)
	{
		m_pAudio->PlayMusic(AK::SWITCHES::MUSIC::SWITCH::PS_CREDITS);
	}
}
...
{
	RenderEngine::RasterTexture( TextureManager::GetInstance()->GetTexture(m_pCreditTexture), 0.0f, 0.0f, 1.0f, 1.0f, 0.0f );
	RenderEngine::RasterTexture( TextureManager::GetInstance()->GetTexture(m_pCreditTexture), 0.0f, m_fCurrentTop - 3.0f, 1.0f, m_fCurrentTop );
}
...
void CCreditsState::Shutdown(void)
{
	//if(m_pAudio)
	//{
	//	//m_pAudio->StopMusic(AK::SWITCHES::MUSIC::SWITCH::PS_CREDITS);
	//}
